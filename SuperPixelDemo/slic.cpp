		...
		for( int n = 0; n < numk; n++ )
		{
			...
			for( int y = y1; y < y2; y++ )
			{
				for( int x = x1; x < x2; x++ )
				{
					...
					//double dist = distlab[i]/100+distxy[i]*invxywt;		//通过改变distlab的分母改变superpixel的规整系数
					//------------------------------------------------------------------------
...
cv::Mat SLIC::GetImgWithContours(cv::Scalar color)
{
	if (type == GRAY) {
		...
		memcpy(result.data, bufferGray, m_width*m_height*sizeof(ushort));
		return result;
	}
	else if (type == RGB) {
...
void SLIC::Mat2Buffer(const cv::Mat& img, ushort*& buffer)
{
	...
	bufferGray    = new ushort[sz];

	memcpy( bufferGray, (ushort*)img.data, sz*sizeof(ushort) );
